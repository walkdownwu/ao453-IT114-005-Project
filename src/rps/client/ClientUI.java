...
public class ClientUI extends JFrame implements IClientEvents {
    ...
    public void showPanel(String name) {
        SwingUtilities.invokeLater(() -> cardLayout.show(cardPanel, name));
    }
    ...
    public void onMessageReceived(long clientId, String message) {
        SwingUtilities.invokeLater(() -> {
            gamePanel.addEvent(message);
            readyPanel.addMessage(message);
        });
    }
    ...
    public void onResetUserList() {
        SwingUtilities.invokeLater(() -> gamePanel.clearUserList());
    }
    ...
    public void onRoundTimer(int seconds) {
        SwingUtilities.invokeLater(() -> gamePanel.updateTimer(seconds));
    }

    @Override
    public void onRoomList(List<RoomPayload.RoomInfo> rooms) {
        SwingUtilities.invokeLater(() -> lobbyPanel.updateRoomList(rooms));
    }

    @Override
    public void onLeaderboard(List<LeaderboardPayload.LeaderboardEntry> entries) {
        SwingUtilities.invokeLater(() -> lobbyPanel.updateLeaderboard(entries));
    }
