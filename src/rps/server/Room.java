...
public class Room {
    ...
    private void syncExistingClients(ServerThread newClient) {
        for (ServerThread existing : clients) {
            if (existing != newClient) {
                newClient.sendClientSync(existing.getClientId(), existing.getClientName());
            }
        }
    }
    ...
    protected void sendToAllClients(Function<ServerThread, Boolean> action, ServerThread exclude) {
        List<ServerThread> snapshot;
        synchronized (this) { snapshot = new ArrayList<>(clients); }
        for (ServerThread st : snapshot) {
            if (st != exclude) action.apply(st);
        }
    }
