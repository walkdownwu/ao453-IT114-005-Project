...
public class Server {
    ...
    public synchronized Room getRoom(String name) { return rooms.get(name); }
    public synchronized void handleCreateRoom(ServerThread client, String roomName, boolean isPrivate) {
        if (rooms.containsKey(roomName)) { client.sendMessage(-1, "Room already exists."); return; }
        GameRoom newRoom = new GameRoom(roomName, isPrivate);
        rooms.put(roomName, newRoom);
        Room oldRoom = client.getCurrentRoom();
        if (oldRoom != null) oldRoom.removeClient(client);
        newRoom.addClient(client);
        broadcastRoomList();
    }
    public synchronized void handleJoinRoom(ServerThread client, String roomName) {
        Room room = rooms.get(roomName);
        if (room == null) { client.sendMessage(-1, "Room not found."); return; }
        if (room == client.getCurrentRoom()) return;
        Room oldRoom = client.getCurrentRoom();
        if (oldRoom != null) oldRoom.removeClient(client);
    ...
    public synchronized void broadcastRoomList() {
        List<RoomPayload.RoomInfo> roomInfos = rooms.values().stream()
            ...
            .map(r -> { Phase phase = (r instanceof GameRoom) ? ((GameRoom) r).getCurrentPhase() : Phase.WAITING;
                return new RoomPayload.RoomInfo(r.getName(), r.getPlayerCount(), r.isPrivate(), phase); })
            .collect(Collectors.toList());
        Room lobby = rooms.get(Constants.LOBBY);
        if (lobby != null) lobby.sendToAllClients(st -> st.sendRoomList(roomInfos), null);
    }
